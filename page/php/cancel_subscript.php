<?php
  ...
  $code = substr(str_shuffle(str_repeat("0123456789ABCDEFGHIJKLMNOPQRSTUVWXZY", 6)), 0, 6);
  //echo $code;

  try {
    $sql_select = $conn->query("SELECT * FROM subscript WHERE username = '$username1'");
    foreach ($sql_select as $row) {
      $time = $row['update_time'];
      $time_2 = $row['update_time_2'];
    }
    ...
    // $t = $second;
    //echo $elapsed;
    if(new DateTime()  < new DateTime($time_2)){
      echo "1";
    }
    else{
      $sql_user = $conn->query("SELECT * FROM user WHERE username = $username1");
      foreach ($sql_user as $row_user) {
        $email = $row_user['email'];
      }
      $newtimestamp = date('Y-m-d H:i:s',strtotime('+4 minutes',strtotime($time_2)));
      //$newtimestamp->format("y-m-d H:i:s");
      $sql = $conn->query("UPDATE subscript SET cancel_code = '$code' , update_time_2 = '$newtimestamp' WHERE username = $username1");
      if($sql == true){
        //echo "4";
        // ---------------- SEND MAIL FORM ----------------
        ...
        $subject="Let cook - Cancel subscript";
              ...
              <div class='col s12 m6'>
                <div class='card blue-grey darken-1'>
                  <div class='card-content white-text center-align'>
                    ...
                    <center>
                    <span class='card-title'><h2>Cancel subscript</h2></span>
                    <h3>This is your code: $code</h3>
                    <br><br>
                    </center>
                  </div>
                </div>
